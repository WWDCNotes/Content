...
- if needed, they prepend your framework name to headers, e.g. <kbd>Cat.h</kbd> will change to <kbd>PetKit/Cat.h</kbd>, however this can cause issues with Clang modules
  ...
  - you don't need to to do this for project headers
...
- within a framework that mixes Swift and Objective-C code, the importer finds declarations in the **umbrella header** (this header defines the public interface, this way, Swift code inside the framework can call public Objective-C code in the same framework)
...
- if you'd like to check/preview how your Objective-C code will be imported into Swift: go to Xcode's related items (<kbd>^</kbd> + <kbd>1<kbd>) > <kbd>Generated interface</kbd>
