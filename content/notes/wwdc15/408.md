...
In our test code, what happens if we add the following line of code?
...
By decoupling the document model from a specific renderer, we were able to plug in a debug renderer that reveals everything our code does in detail. Decoupling with protocols + generics makes our code much more testable.
...
The code looks like this. Notice how the `startAngle` and `endAngle` parameters are repetitive when placed next to Circle.
...
To resolve this repetition, we can just add a new requirement to the `Renderer` protocol and then update our models to maintain conformance:
...
However, `TestRenderer` and `CGContext` also have a duplicate implementation. We still have repeated code! 
...
Protocols define requirements which can be immediately fulfilled in extensions. This creates a customization point. To see how this plays out, let's see what happens when we add another method to the extension that isn't part of the requirements:
...
The result isn't surprising; `circle(at:radius:)` and `rectangle(at:)` use the implementations inside `TestRenderer` instead of the default implementations from the `Renderer` protocol extensions. We would get the same result if we removed the conformance.
...
You could fix this by adding a conformance to `Ordered` for `Int`, `String`, and so on, but that isn't maintainable. Instead, we can do a single constrained extension that provides a default implementation for any type that implements `Comparable`:
...
Now, because `Int`, `String`, and `Double` all conform to `Comparable`, we can perform binary search on an array of any `Comparable` type.
...
- Instances are just "sinks"
  - Here, a "sink" means something that just receives state (like our renderer, or `CGContext`).
