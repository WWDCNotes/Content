...
- session is capable to detect if the scanned object needs to flip to scan parts not visible by the camera yet, for example, the bottom. You can figure that out through `ObjectCaptureSession.Feedback` and calling `session.feedback.contains(.objectNotFlippable)`:
  ...
  ```
  If the user decides to flip the object, then `beginNewScanPassAfterFlip()` will set the session to `ready` state to adjust the bounding box to the new orientation. Otherwise, we must use `beginNewScanPass()` to keep in `capturing` state and keep the same bounding box.
