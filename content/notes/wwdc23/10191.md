...
- To get started, you first need to import RealityKit and SwiftUI, and then create a session object:
  ```swift
  import RealityKit
  import SwiftUI 
...
- Next, we create an `ObjectCaptureView` providing session object. This view must be wrapped inside another SwiftUI view.
  ```swift
  import RealityKit
  import SwiftUI
...
- To start detecting the object, you need to call `session.startDetecting()` from the UI:
  ```swift
  var body: some View {
      ZStack {
          ObjectCaptureView(session: session)
  ...
  ```
  This state will show the bounding box that allow you to adjust the capture edges.
    ...
    ```
- Once you are done adjusting the bounding box, call `session.startCapturing` from the UI to move to the next state:
  ```swift
    var body: some View {
      ZStack {
          ObjectCaptureView(session: session)
  ...
  ```
  The session will automatically take images while you slowly move around the object.` ObjectCaptureView` will provide a dial indicating the areas that need scanning.
- Once the scanning is complete, `session.userCompletedScanPass` is set to `true`:
  ...
  ```
- `session` is capable to detect if the scanned object needs to flip in order scan parts not visible by the camera yet, for example, the bottom. You can figure that out through `ObjectCaptureSession.Feedback` and calling `session.feedback.contains(.objectNotFlippable)`:
  ```swift
  ...
  ```
  If the user decides to flip the object, then `beginNewScanPassAfterFlip()` will set the session to `ready` state to adjust the bounding box to the new orientation.
  Otherwise, we must use `beginNewScanPass()` to keep in `capturing` state and start capturing from different heights while keeping the same bounding box.
...
- Once finished, the session will automatically move to the `completed` state. This will start the on-device reconstruction.
  > In case of an error happens during the process, the session will enter into `failed` state and a new session will need to be created.
...
Here's the code in the sample app that start the reconstruction process. It will work for both iOS and macOS:
```swift
var body: some View {
...
- By default, `ObjectCaptureSession` will stop capturing images when the reconstruction limit for the iOS device is reached.
- For macOS, you can allow the session to take more images than on-device reconstruction can use by setting `isOverCaptureEnabled` to `true` in session's configurations:
  ```swift
  var configuration = ObjectCaptureSession.Configuration()
...
- You don't need any single line of code, just import the images to Reality Composer Pro, choose a detail level, and get your model!
...
- In addition to progress percent, The API now provides estimated processing time.
