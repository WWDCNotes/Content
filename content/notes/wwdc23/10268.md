...
* At build time, the static linker finds which APIs to use from those libraries and copies that code into the app binary -> since it's copied, *the library isn't needed after building*.
...
* Any frameworks not in the Apple SDK must be embedded into the app bundle. This leads to increase in memory consumption and app launch time because generally we have hundreds of frameworks. 
