...
- leverags a web standard called [schema.org][schema]
...
- If the event gets cancelled, update the `reservationStatus` to `http://schema.org/ReservationCancelled`
...
- Keep `reservationId` consistent for updates and cancelled reservations
...
- The `INReservation` objects are added to an intent response that, together with the intent, forms the interaction, [`INInteraction`], that the app donates to Siri.
